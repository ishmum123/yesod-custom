...
spec = withApp $ do
    describe "valid request" $ do
        ...
        it "gives a 200 (raw string request)" $ do

            let message = "String JSON Message" :: Text

            request $ do
                setMethod "POST"
                setUrl CommentR
                setRequestBody "{\"message\": \"String JSON Message\"}"
                addRequestHeader ("Content-Type", "application/json")
    ...
    describe "invalid requests" $ do
        ...
        it "422s when the JSON structure is invalid" $ do
            get HomeR

            request $ do
                setMethod "POST"
                setUrl CommentR
                setRequestBody "{\"message\" \"Invalid JSON (without colon)\"}"
                addRequestHeader ("Content-Type", "application/json")
